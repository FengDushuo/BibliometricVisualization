...
def run_models(input_file,output):
    ...
    """
    从 CSV 文件中加载 BERT 的模型结果
    """
    data = pd.read_csv("output_data/algorithm_data_file_old.csv")
    bert_data = data[data['Algorithm Name'] == 'BERT']  # 筛选 BERT 相关数据
    methods_list.append({
        "Algorithm Name": "BERT",
        "Train Score": bert_data.iloc[0]['Train Score'],
        "Test Score": bert_data.iloc[0]['Test Score'],
        "Accuracy": bert_data.iloc[0]['Accuracy'],
        "Balanced Accuracy": bert_data.iloc[0]['Balanced Accuracy'],
        "Precision": bert_data.iloc[0]['Precision'],
        "Recall": bert_data.iloc[0]['Recall'],
        "F1 Score": bert_data.iloc[0]['F1 Score'],
        "MCC": bert_data.iloc[0]['MCC'],
        "Cohen's Kappa": bert_data.iloc[0]["Cohen's Kappa"],
        "ROC AUC": bert_data.iloc[0]['ROC AUC']
    })      

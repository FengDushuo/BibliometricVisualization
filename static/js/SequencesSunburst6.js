  ...
  var file = files_path + timeline+'.json';
...
// var colors = ["#66CCCC","#FF99CC","#FF9999","#99CC66","#5151A2","#FF9900","#FF6600","#CCFF66"];
  ...
  var draw = function draw() {
    ...
    var  emblemText= ['T I M E L I N E', 'T I M E L I N E', 'T I M E L I N E'];    
    circleText('.emblem');
...
function piesingle (e) {
  ...
  timeOut = setTimeout(function () {
      ...
      .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
      .duration(secDur);
      file = files_path+timeline+'-'+e.srcElement.nextSibling.lastChild.innerHTML+'.json'; 
      (async () => {
        var data = await d3.json(file).then(data => data);
        generateChart(data);
      })();
...
function piedouble (e) {
  ...
  .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
  .duration(secDur);
  var file1 = files_path+timeline+'-'+e.srcElement.nextSibling.lastChild.innerHTML+'.json'; 
  var names1 = []; //类别数组（用于存放饼图的类别）
  var brower1 = [];
  $.ajax({
    url: files_path+"count_timeline.json",
...
timeOut = setTimeout(function () {
    ...
    .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
    .duration(secDur);
    file = files_path+timeline1+'-'+e.srcElement.nextSibling.lastChild.innerHTML+'.json'; 
    (async () => {
      var data = await d3.json(file).then(data => data);
      generateChart1(data);
    })();
...
clearTimeout(timeOut); // 清除第二个单击事件
var file1 = files_path+timeline1+'-'+e.srcElement.nextSibling.lastChild.innerHTML+'.json'; 
var names1 = []; //类别数组（用于存放饼图的类别）
var brower1 = [];
const svg = d3.select('#bubble-chart');
svg.selectAll("g").remove()

  ...
  var city = param_json.countries[0]
  var file = files_path + city+'.json';
  var count_file = files_path + 'count_country.json';
...
// var colors = ["#66CCCC","#FF99CC","#FF9999","#99CC66","#5151A2","#FF9900","#FF6600","#CCFF66"];
  ...
  var draw = function draw() {
    ...
    var  emblemText= ['Visualized item', 'Visualized item', 'Visualized item'];    
    circleText('.emblem');
    ...
    dataq.forEach(({value, text, color}, index) => {
      ...
      let texts = ["research field"];
      if (width > len)
        texts = splitLongString("research field", Math.ceil(width / len));
  ...
  function single (e) {
      ...
      timeOut = setTimeout(function () {
          ...
          duration(secDur);
          file = files_path + e.data.name.replace("&","and").replace("/","or")+'.json'; 
          city = e.data.name;
          var names1 = []; //类别数组（用于存放饼图的类别）
          var brower1 = [];
          $.ajax({
            url: files_path+"count_country.json",
            data: {},
            type: 'GET',
            success: function(data) {
                //请求成功时执行该函数内容，result即为服务器返回的json对象
                $.each(data, function(index, item) {
                  if (param_json.countries.includes(item.country)) {
  ...
  function double (e) {
      ...
      .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
      .duration(secDur);
      var file1 = files_path+e.data.name.replace("&","and").replace("/","or")+'.json'; 
      city1 = e.data.name
      var names1 = []; //类别数组（用于存放饼图的类别）
      var brower1 = [];
      $.ajax({
...
function piesingle (e) {
  ...
  timeOut = setTimeout(function () {
      ...
      .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
      .duration(secDur);
      file = files_path+city+'-'+e.srcElement.nextSibling.lastChild.innerHTML.replace("&","and").replace("/","or")+'.json'; 
      (async () => {
        console.log(file);
        var data = await d3.json(file).then(data => data);
        generateChart(data);
...
function piedouble (e) {
  ...
  .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
  .duration(secDur);
  var file1 = files_path+city+'-'+e.srcElement.nextSibling.lastChild.innerHTML.replace("&","and").replace("/","or")+'.json'; 
  var names1 = []; //类别数组（用于存放饼图的类别）
  var brower1 = [];
  $.ajax({
    url: files_path+"count_country.json",
...
timeOut = setTimeout(function () {
    ...
    .delay(function (d, i) {return arcAnimDur + i * secIndividualdelay;})
    .duration(secDur);
    file = files_path+city1+'-'+e.srcElement.nextSibling.lastChild.innerHTML.replace("&","and").replace("/","or")+'.json'; 
    (async () => {
      console.log(file)
      var data = await d3.json(file).then(data => data);
      generateChart1(data);
...
var file1 = files_path+city1+'-'+e.srcElement.nextSibling.lastChild.innerHTML.replace("&","and").replace("/","or")+'.json'; 
var names1 = []; //类别数组（用于存放饼图的类别）
var brower1 = [];
const svg = d3.select('#bubble-chart');
svg.selectAll("g").remove()
              ...
              label: {
                  ...
                  normal: {
                      ...
                      formatter: (params) => {
                          ...
                          const wrappedName = name.match(/.{1,12}/g)?.join('\n');
                          return wrappedName || name;
                      },
                      textStyle: { //标签的字体样式
                        color: '#000000', //字体颜色
